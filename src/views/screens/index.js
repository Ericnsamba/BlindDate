// ath
export {default as SplashScreen} from './SplashScreen';
export {default as SignInScreen} from './Auth/SignInScreen';
export {default as SetUserNameScreen} from './Auth/SetUserNameScreen';

//home
export {default as HomeTabScreensFirst} from './HomeTabScreens/First';
export {default as HomeTabScreensSecond} from './HomeTabScreens/Second';
export {default as HomeTabScreensMain} from './HomeTabScreens/Main';
export {default as LastTabScreensMain} from './LastTabScreens/Main';

//profile
export {default as ProfileScreen} from './ProfileTabScreens/Profile';
export {default as ProfileMenu} from './ProfileTabScreens/ProfileMenu';
export {default as ProfileInterests} from './ProfileTabScreens/ProfileInterests';
export {default as Invites} from './ProfileTabScreens/Invites';

//chat
export {default as ChatScreen} from './ChatScreen/Main';
export {default as Messages} from './ChatScreen/Messages';
export {default as Threads} from './ChatScreen/Threads';
